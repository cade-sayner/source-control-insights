...
public class Commands {
    ...
    public String updateRepo(@ShellOption(value="-r") String repoId, @ShellOption(value = "-p", defaultValue = "") String path) {
        ...
        try {
            if (!loginService.isValidToken(this.token))
                return "You must be logged in to access this command";
            String repoPath = path.isEmpty() ? this.commandOutputs.getRepoLocalDirectory(): path.strip();
            return authenticatedApiClient.updateRepo(repoId, GitLogFetcher.getGitLogAsCSV(repoPath.replaceAll("\\\\", "/").strip()));
        } catch (Exception _) {
